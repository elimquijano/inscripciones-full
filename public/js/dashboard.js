...
/* $(document).ready(function () {
    ...
    let datosInscripcion = $.ajax({
        url: window.location.origin + "/api/inscripcion",
        type: "GET",
    });

    // Segunda consulta AJAX
    let datosOtraConsulta = $.ajax({
        url: window.location.origin + "/api/pago",
        type: "GET",
    });

    // Cuando ambas consultas AJAX se hayan completado
    $.when(datosInscripcion, datosOtraConsulta).done(function (
        respuestaInscripcion,
        respuestaOtraConsulta
    ) {
        // Procesa los datos de las respuestas
        let datosInscripcion = respuestaInscripcion[0];
        let datosOtraConsulta = respuestaOtraConsulta[0];

        // Procesa los datos
        datosInscripcion = datosInscripcion.map(function (inscripcion) {
            // Inicializa 'verificado' como 'No'
            let verificado = "No";

            // Si datosOtraConsulta no está vacío, busca coincidencias
            if (datosOtraConsulta?.length > 0) {
                let coincide = datosOtraConsulta.some(function (otra) {
                    return (
        ...
        tablaInscripciones.on("init.dt", function () {
            ...
            $("#table-inscripciones thead th").each(function (i) {
                ...
                $("input", fila.children().last()).on(
                    "keyup change",
                    function () {
                        if (
                            tablaInscripciones.column(i).search() !== this.value
                        ) {
                            tablaInscripciones
                                .column(i)
                                .search(this.value)
